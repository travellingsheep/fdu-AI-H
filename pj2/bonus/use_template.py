...
if __name__ == '__main__':
    ...
    if language == "English":
        train_file_path = r".txt"
        evaluate_file_path = r".txt"
        train_dataset = read_data(train_file_path)
        evaluate_dataset = read_data(evaluate_file_path)
        model_dir = r"task2"
        gold_path = r"validation.txt"
    else:
        train_file_path = r"train.txt"
        evaluate_file_path = r"validation.txt"
        train_dataset = read_data(train_file_path)
        evaluate_dataset = read_data(evaluate_file_path)
        model_dir = r"task2"
        gold_path = r"validation.txt"
    ...
    word_to_index = {"<UNK>": 0}
    label_to_index = {"O": 0, "<START>": 1, "<STOP>": 2}
    for _, labels in train_dataset:
        for label in labels:
            if label not in label_to_index:
                label_to_index[label] = len(label_to_index)

    for words, _ in train_dataset:
        for word in words:
            if word not in word_to_index:
                word_to_index[word] = len(word_to_index)

    num=11
    model = CRF_Perceptron_Dynamic(label_to_index, word_to_index, templates=templates)
    for epoch in range(1,21):
        print(f"No.{epoch}")
        best_f1 = train_online(model, train_dataset, evaluate_dataset, 1, language, gold_path, model_dir)
        my_path = os.path.join(model_dir, f"validation_auto{num}_epoch={epoch}.txt")
        save_result(evaluate_dataset, model, my_path, language = language)
        result = check(language, gold_path, my_path)
        print(f"Final result: {result}")

        with open(f"{model_dir}/crf_perceptron_model_auto{num}_epoch={epoch}_avg_score={result}.pkl", "wb") as f:
            pickle.dump({
                'bigram_params': model.bigram_params,
                'u_feature_params': model.u_feature_params,
                'b_feature_params': model.b_feature_params,
