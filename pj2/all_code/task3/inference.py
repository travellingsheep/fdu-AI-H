...
def train_model(sample_data,language,device,epochs):
    ...
    print(f"label_nums: {len(label_to_idx)}")
    print(f"labels: {list(label_to_idx.keys())}")
    print(f"vocab_size: {len(char_to_idx)}")
    ...
    model = TransformerCRF(vocab_size, num_tags, d_model=64, nhead=8, num_layers=4)
    model.to(device)
...
def load_model_with_vocab(filepath, d_model=64, nhead=8, num_layers=4, max_len=256, dim_feedforward=2048):
    """加载包含词汇表的完整模型"""
    ...
    return model, char_to_idx, label_to_idx, idx_to_label

def save_result(dataloader, model, idx_to_label, device, output_file):
    model.eval()
    model.to(device)
    sample_idx = 0
    with open(output_file, 'w', encoding='utf-8') as f:
        with torch.no_grad():
            for batch in dataloader:
                ...
                for i, seq_len in enumerate(seq_lens):
                    pred_labels = predictions[i][:seq_len]
                    pred_label_names = [idx_to_label[idx] for idx in pred_labels]
                    chars = dataloader.dataset.data[sample_idx][0][:seq_len]
                    for char, label in zip(chars, pred_label_names):
                        f.write(f"{char} {label}\n")
                    f.write("\n")
                    sample_idx += 1

if __name__ == "__main__":
    model_path = r"Chinese_model.pth"
    model, char_to_idx, label_to_idx, idx_to_label = load_model_with_vocab(model_path)

    new_validation_data = read_data(r"chinese_test.txt")
    max_len = 256
    new_val_dataset = NERDataset(new_validation_data, char_to_idx, label_to_idx, max_len=max_len)
    ...
    output_file = r"new_validation_prediction.txt"
    save_result(new_val_loader, model, idx_to_label, device, output_file)
