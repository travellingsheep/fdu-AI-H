...
def save_result(dataset, model, output_file):
...
if __name__ == "__main__":
    ...
    if language == "English":
        ...
        train_file_path = r"Chinese\train.txt"
        evaluate_file_path = r"Chinese\validation.txt"
        train_dataset = read_data(train_file_path)
        evaluate_dataset = read_data(evaluate_file_path)
        model_dir = r"Chinese"
        gold_path = r"Chinese\validation.txt"
    
    smoothing=np.arange(0.001,0.01,0.001)
    for i in range(0,len(smoothing)):
        print(i)
        model = HMM(smoothing[i])
        model.train(train_dataset)
        my_path = os.path.join(model_dir, f"date=0527_test_num=5_formal_smoothing={smoothing[i]:.3f}.txt")
        save_result(evaluate_dataset, model, my_path)
        result = check(language, gold_path, my_path)
        model_path = os.path.join(model_dir, f"date=0527_test_num=5_formal_smoothing={smoothing[i]:.3f}_acc={result}.pt")
        model.save_model(model_path)

        log_path = os.path.join(model_dir, "date=0527_param_stats_log.csv")
        header = "date,test_num,smoothing,accuracy\n"
        log_line = f"0527,5,{smoothing[i]:.3f},{result}\n"
